...
{	
	...
	{		
		{
			...
			double actual = GraphImpl.calcUnilateralLevelOfConvergence(p1, p1Velocity,
					p2);
			assertTrue(actual > 0);

			actual = GraphImpl.calcUnilateralLevelOfConvergence(p2, p2Velocity,
					p1);
			assertTrue(actual < 0);
		}
...

		{
			...
			actual = GraphImpl.calcUnilateralLevelOfConvergence(p2, p2Velocity, p1);
			assertEquals(0, actual, 0.00001);
		}

		{
			...
			double actual = GraphImpl.calcUnilateralLevelOfConvergence(p1, p1Velocity, p2);
			assertEquals(0, actual, 0.00001);
	...
	public void diverganceTest() 
	{
		...
		double convergence1 = GraphImpl.calcUnilateralLevelOfConvergence(p1, p1Velocity, p2);

		double convergence2 = GraphImpl.calcUnilateralLevelOfConvergence(p2, p2Velocity, p1);
		
		p1Velocity = new PolarCoordinate(Math.PI, 1.0);
		p2Velocity = new PolarCoordinate(0.0, 1.0);

		double divergence1 = GraphImpl.calcUnilateralLevelOfConvergence(p1, p1Velocity, p2);

		double divergence2 = GraphImpl.calcUnilateralLevelOfConvergence(p2, p2Velocity, p1);
