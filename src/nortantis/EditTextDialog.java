...
public class EditTextDialog extends JDialog
{
	...
	private void handleUpdate(MapText selectedText)
	{
		...
		if (lastSelected != null && !editTextField.getText().equals(lastSelected.text))
		{
			...
			{
				// If the user added text, then edited it, then this will replace the MapText object created b
				// createUserAddedText (see handleMouseClickOnMap) with the one generated by TextDrawer.
				settings.edits.addedText.put(lastSelected.id, lastSelected);
			}
			else
			{
