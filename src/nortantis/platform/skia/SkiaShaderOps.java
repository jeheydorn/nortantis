...
public class SkiaShaderOps
{
	...
	/**
	 * Creates a 2D palette texture from the colors map using the Image wrapper. Region ID r is stored at pixel position (r % 256, r / 256). Used by colorifyMulti and maskWithMultipleColorsInPlace
	 * methods.
	 */
	...
	/**
	 * Determines whether the given images should run on the GPU. Returns true if GPU is available, all images are SkiaImages, and at least one image is GPU-backed. Small images that aren't GPU-backed
	 * will be uploaded temporarily during shader execution.
	 */
	...
	/**
	 * Returns true if Skia shaders should be used for the given images. This is true when shaders are enabled and all images are SkiaImages. Shaders will run on GPU if available, otherwise on CPU
	 * using Skia's optimized CPU rasterizer.
	 */
	...
	/**
	 * Checks if all images are SkiaImage instances and at least one has GPU enabled. This allows shader operations to run on GPU even when some images (like small masks) are below the GPU threshold -
	 * they will be uploaded temporarily.
	 */
