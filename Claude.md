...
- **Scroll panes:** Every `JScrollPane` you create needs `scrollPane.getVerticalScrollBar().setUnitIncrement(SwingHelper.sidePanelScrollSpeed)`. Swing's default unit increment is 1 pixel, which makes the mouse wheel crawl. (A `Scrollable` view can supply the increment itself instead — see `VerticallyScrollablePanel`.)
...
- **Unused imports:** Do remove imports your own changes made unused. But when reviewing my code, don't report unused imports as findings — IntelliJ flags those for me already.
